...
fn validate_command(
    ...
    for (i, name) in names.iter().enumerate() {
        match value[i] {
            // ref cmd 会创建一个名为cmd的不可变引用
            RespFrame::BulkString(ref cmd) => {
                // cmd.as_ref() 获取了这个cmd字符串 slice 的一个引用，转为小写后再和 name 做比较
...
fn extract_args(value: RespArray, start: usize) -> Result<Vec<RespFrame>, CommandError> {
    // into_iter() 会 consume 这个 value，最终再 collect 成 Vec<RespFrame>
